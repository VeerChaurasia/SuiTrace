...
func main() {
	...
	for {
		...
		// Stop if we've reached the pagination limit or no more cursor
		cursor = nextCursor
		if cursor == nil {
		...
		// For testing purposes, limit to 200 events
		if totalFetched >= 200 {
			fmt.Println("Reached test limit of 200 events")
			break
		}
	}
	...
	err := saveEventsToCSV(allEvents, "events.csv")
	if err != nil {
		log.Fatalf("Failed to save events to CSV: %v", err)
	}
	
	fmt.Printf("Done! %d events saved to events.csv 🎉\n", len(allEvents))
}